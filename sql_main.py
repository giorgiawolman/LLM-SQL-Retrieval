...
print(f"Relevant table identified: {relevant_table}")
...
if "No information" in sql_query:
    print("LLM indicates the question cannot be answered with the current dataset.")
    exit()
...
print("Debug: Feature columns:", X.columns)

if "comfort_score" in X.columns:
    X = X.drop(columns=["comfort_score"])
else:
    print("Warning: 'comfort_score' not found in features. Skipping drop.")
